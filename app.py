...
def inserirA():
    ...
    if request.method == 'POST':
        ...
        erro = "Article successfully inserted"  # Artigo inserido com sucesso
    ...
    if request.method == 'POST':
        if art.id:
            if "cancel" in request.form:
                ...
                art.apaga(art.id)
                erro = "Article deleted successfully"  # Artigo eliminado com sucesso

            elif "edit" in request.form:
                v1 = request.form['price']
                art.alterar(art.id, v1)
                art.select(art.id)  # Atualizar os dados na classe
                erro = "Price changed successfully"  # Preço alterado com sucesso
...
def eliminarA():
    if request.method == 'POST':
        v1 = request.form['id']
        title = "Deleting your Article"  # Eliminação do seu Artigo

        return render_template('accounts.html', title=title, accounts=art.listaA(v1), campos=art.campos, usr=usr)

    erro = "Indicate the id of your Article to be deleted."  # Indique o id do seu Artigo a eliminar.
